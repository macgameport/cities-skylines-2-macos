...
| Boot-verify (**the** way to prove the game boots) | `scripts/boot-verify.sh` — launcher → dwell → `WM_CLOSE` → judge `Logs/SceneFlow.log` (flushed only on a graceful exit). Prints `GRACEFUL: yes\|no` and one `VERDICT: PASS\|FAIL\|VOID` line; the exit code follows (2 = refused, the prefix is busy). **Run it detached with a log** — a tool timeout kills the process group and Steam with it. `--selftest` = every judge branch, no launch. Run dirs: `~/cs2-patch/boot-verify/` (outside the evidence store on purpose: the ledger checker treats every store dir as a cell) |
