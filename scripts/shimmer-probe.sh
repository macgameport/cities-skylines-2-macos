...
/tmp/darkboxes 6 "$out"/f*.png | awk '{split($5,l,"=");split($6,r,"=");split($7,t,"=");split($8,b,"="); m=l[2]+0; if(r[2]+0>m)m=r[2]+0; if(t[2]+0>m)m=t[2]+0; if(b[2]+0>m)m=b[2]+0; if(m>=20)n++; if(m>mx)mx=m} END{printf "  EXPOSED-EDGE frames (an outer-10%% band >=20%% true black, lum<6): %d of %d, worst band %.0f%%\n", n+0, NR, mx+0; if(n>0) print "  -> the hosted child lagged the resize and the host background showed; a clean interior does NOT clear the run (issue #7)"}'
