...
| `livedrag-probe.sh` | The same measurement during a **drag**. Churn is not a drag — a drag runs win32u's `SC_SIZE` loop, not `SetWindowPos` from outside (and not macOS live-resize either, C46) — so anything gated on that loop is invisible to `shimmer-probe` and only this can see it. Waits for the window to start changing size (a hand or `sizedrag`), samples hard while it does, and returns VOID rather than a clean bill if the size never actually changed. Under a locked session it wakes the display and lets its known-good capture decide: a window capture works whenever the display is awake, and only display sleep blinds it (2026-09-05). **`CAPTURE=screen`** is the issue #12 control: the same rect off the COMPOSITED display (`-R`) instead of the window server's stored representation (`-l`), so a frame black in `window` and lit in `screen` is a capture artifact nobody saw, and black in both is a real display gap. Both modes return identical pixel dimensions, so the band metrics compare directly; `-R` is the faster call (88 vs 113 ms). Screen mode alone can be fooled by a locked session or a window in front of Steam, so it checks and records both, before and after sampling, and calls the frames VOID rather than reading them. ⚠ **Its EDGE numbers are not comparable with window mode's**: the region is frozen at the rect read before the capture and the window grows 25-75 px (mean 28) under it during the ~88 ms, on the same growing frames issue #7's strip appears on, so screen mode crops the strip out of its own right band. `frames/rects.txt` records the before/after size per frame. A whole-host signature (a 100 % black top band, issue #12) is unaffected |
| `drag-session.sh` | One drag measurement set up end to end — installs the right module, brings Steam up on the store page, pre-sizes so the drag can grow, waits (or drags), scores, and restores the daily driver on exit. `t0` \| `t2b` \| `t3` per issue #7's three drags, `s1` = the stage-1 daily driver. `DRAG=synth` runs the size loop through `win-resize-driver.exe sizedrag` with nobody at the mouse — the same win32u path a human drag takes here (C46/C47); a human drag stays the acceptance test. `SYNTH_PX` `SYNTH_MS` `SYNTH_REPEAT` `SYNTH_PAUSE` `PRESIZE` shape the synthetic drag — coarse cadences reproduce the strip, the fine one never did (C50) — and a press that does not take is re-pressed once (C48) |
...
| `band-counts.py` | Counts frames whose outer 10 % band is ≥ 20 % true black, **per edge**. Parses by label, never by position. With the probe's `sizes.txt` beside the frames it also prints the right band's mean and max over growing frames, the A/B metric the threshold count is too coarse for (C50) |
...
| `t6-scale-at-rest.py` | Issue #7 T6 — no *surviving* hosted layer is left scaled once the resize is over. The surviving filter is the whole test: a churn retires hundreds of contexts and a retired host's last placement is frequently non-identity, which is the fix working, not failing. A supersede AFTER the last placement counts too: CEF's final re-create lands 21-65 lines later and the CREATE handler traces no placement line, so cutting there turned every drag's settle into a FAIL (four runs, 2026-09-05); with `+timestamp` it must land within 3 s |
