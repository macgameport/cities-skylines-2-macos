...
>
> **2026-09-05 — T6 was never failing, S-A is closed, a synthetic instrument exists, and it found
> something.** (1) Every T6 FAIL on a live drag was `t6-scale-at-rest.py` cutting deaths at the
> last placement, 21–65 lines before CEF's final re-create superseded the scaled host (ledger
> **C49**); re-scored PASS on all four, the scorer now counts a supersede whenever it occurs, and
> every drag row traces `+timestamp`, so the settle is measured: 6–17 ms across the night's runs.
> C45's T6 half is retracted, C46's sentence corrected. (2) The C48 battery's S-A press never took —
> an app activation mid-loop made SDL toggle the capture and win32u ended the loop; the re-run is
> clean on every axis (**C48** → SUPPORTED) and `drag-session.sh` re-presses once. (3) **A coarse
> synthetic drag reproduces the strip** (**C50**): 8 px / 16 ms, 25 px / 120 ms and 50 px / 200 ms
> show it on the pre-stage-1 baseline (2 · 4 · 3 of 60; growing-frame right-band mean 16.5 · 11.6 ·
> 8.3 %), stage 1 shows 0 · 0 · 0 (6.6 · 5.4 / 1.4 · 1.4 %), and stage 2 at 25 / 120 sits inside
...
> (4) ⚠ **SUPERSEDED 2026-09-06 by C54 and C55 — read those first.** C55: the black frame occurs on the **pre-stage-1 baseline** too, so it is architectural and neither stage introduced it. **C56 names the mechanism: S3 — the child's own offscreen layer before its first drawable** (`Tblue=100 %` on the diag build), which also settles that it is a real display gap and not a capture artifact. At full drag coverage stage 1 beats baseline with complete separation (p = 0.0079) but **stage 2 is worse than stage 1** (p = 0.0317), and the black frame occurs on the **stage-1 daily driver** too (2 of 5), so it is not a stage-2 defect. Stage 2 has no remaining case for promotion. The original note follows. (4) **Stage 2 shows a one-capture black full-client host** in 2 of 3 prod runs at 25 / 120 and in
...
> must say whether the chrome ever flashes black**; T2b / T3 now read the top band too. (5) **Stage 3
> is declined** (exit criterion 4): the create-path colour is 0 in the right band on churns (C39,
> C42) and on the coarse synthetic drag (`exp_4b7464`: magenta 0, its one exposed frame black with
> no colour); S2 does not reach the edge bands. **Still owed for stage 2:** T3 (James's drag on prod
> `s2b`, verdict verbatim, including the chrome flash) and T11 (popups during and after a drag).
> Exit criterion 3's synthetic rows are met.
>
...
| T2b | **drag, stage 2** — diag-fix module (`s2b-diag`), the fixed drag script: synthetic first (`scripts/stage2-tests.sh` S-B, `sizedrag` through win32u's loop), then James | among grow-step frames: `Rgreen ≥ 20 %` ≤ 1, black-no-colour ≥ 20 % ≤ 1, top band ≥ 50 % in 0 frames (C51), worst right band < 20 %; top band black-no-colour 0 in the top-edge segment; **mechanism (S-A, prod):** every root pass during the drag traces `size/move loop 1`, `stretched … (live resize)` > 0, `macdrv_size_move_ended` once per segment | **E4′ (sig-off, `signal-mutants.py --off`, S-C):** the helper never sees the loop → the same synthetic drag fires 0 stretches with every root pass reading `loop 0`; and E1 |
| T3 | **live drag, production module** (`s2b`) — same script, James's verdict; S-A is the unattended half | right band ≥ 20 % true black in **≤ 2 grow-step frames** (baseline 19/60 on a mixed drag); worst < 20 %; **no capture with the top band ≥ 50 % black** (C51 — the chrome must not flash); verdict verbatim, including the drag-end frame (rubbery ≤ ~100 ms = the native look; persistent distortion or a snap-back flash = fail) | none beyond T2b's — a human drag is not repeated per mutant |
...
| T6 | **no residual scale at rest, and the base is right** — the §4.5 trace; run after T2b's drag and after T7's churns | 3 s after the last size change the last line per surviving context id reads `1.000,1.000`; each surviving host's creation size equals the GPU-process `cross-process child … frame` rect (`window.c:1327`) for the same context id — **not** the host's bounds, which are set from the stored size; no `child NULL` context appears in the reframe trace (`window.c:1937`) | **E3:** store half the creation size → `2.000,2.000` at rest and the page renders doubled **Scorer (2026-09-05):** `t6-scale-at-rest.py` counts a retire-by-supersede whenever it occurs and a RELEASE only up to the last placement; every drag row now traces with `+timestamp`, so a supersede later than 3 s after the last placement counts as a survivor and the delay is printed. The four earlier FAILs (C45, C46, C48's S-A/S-B) were the old cut falling before CEF's final re-create, 21–65 lines below it — ledger **C49**; measured settle on the timed re-run 51–88 ms. |
