...
| C50 | **A synthetic drag reproduces the growing-edge strip once its steps outrun the child's re-create, and on that instrument stage 1 reduces the strip at every cadence** — 8 px / 16 ms, 25 px / 120 ms and 50 px / 200 ms all show it on the pre-stage-1 baseline; 1 px / 16 ms never did on anything | `SUPPORTED` | `drag-session.sh DRAG=synth` with `SYNTH_PX` / `SYNTH_MS`, `SYNTH_REPEAT=3`, `SYNTH_PAUSE=3`, `PRESIZE=1000x650`, right edge +650 px ×3 then top −350; Steam store page; `band-counts.py` (its growing-frame line was added the same night). Cells: baseline `cd79fc463795939f` — `exp_5e29fc` (8/16), `exp_1f0a17` (25/120), `exp_e99836` (50/200) · stage 1 `2a251a4b2510fb84` — `exp_d5fd29` (8/16), `exp_405b45` + `exp_706825` (25/120), `exp_9cc85a` (50/200) · stage 2 `00ac32aa3115b455` — `exp_e5a2a7` + `exp_11a2a3` (25/120), `exp_1fa790` (S-A, 1/16) · stage 2 diag `7bdfcd9c1746f483` — `exp_4b7464` (25/120). Run dirs `~/cs2-patch/drag/k1…k10-*`, frames kept | **Measured — right band ≥ 20 % true black, of 60:** baseline **2 · 4 · 3** at 8/16 · 25/120 · 50/200; stage 1 **0 · 0 (×2) · 0**; stage 2 **1 (×2)** at 25/120, and both of those are the black full-client frame of [[C51]], not a strip. **The finer metric — right band mean / max over GROWING frames (n growing):** baseline 16.5 / 27.4 (6) · 11.6 / 47.8 (13) · 8.3 / 30.1 (8); stage 1 6.6 / 15.9 (6) · 5.4 / 18.2 (15) and 1.4 / 1.7 (15) · 1.4 / 1.7 (8); stage 2 5.2 / 22.9 (13) and 2.8 / 23.0 (16); stage 2 diag 4.3 / 26.6 (12), its one ≥ 20 % frame **black with no colour** (S4 — nothing hosting the area; green, magenta and blue all 0); S-A at 1/16 on stage 2: 1.8 / 2.9 (31). Frames looked at: on the baseline at 25/120 the chrome, page and footer all stop ~75 px short of the window's edge with black beyond; on stage 1 the same strip ~27 px wide. Every press took first time (0 re-presses in 10 runs) once the window stayed under ~1700 px and presses were 3 s apart; the first K1 (`exp_7855af`, captures VOID by the lock) had a shrink take 8 px and two grows not take at all from 1868 px on a 1920 px display. **The probe's 60 captures span 8–9 s** (0.13–0.15 s each), so at these cadences they cover the first grow segment and part of the first shrink; the ×3 repeats fall outside the window. **Inferred, not measured:** that stage 2 adds nothing over stage 1 on this instrument — 5.2 / 2.8 against 5.4 / 1.4 at n = 2 sits inside the run-to-run spread, so the two are *not separable here*, which is not the same as equal; and that stage 1's residual at 25/120 (one step plus lag) is the S4 the churn A/B measured ([[C38]]). **Superseded on the module comparison 2026-09-05 ([[C54]]):** this row's A/B is a first-quarter measurement ([[C53]]). Re-run at full coverage the ordering is baseline 6.3 % · stage 1 3.1 % · stage 2 4.7 % (medians): stage 1 beats baseline with complete separation (p = 0.0079) — this row's direction confirmed — but stage 2 is **worse than stage 1** (p = 0.0317), not "not separable". **Not a substitute for T3:** the hand's 19/60 on the baseline ([[C35]]) is still four times this instrument's best, and only a hand has seen the strip on stage 1. **What would overturn it:** a baseline run at a coarse cadence with no growing-frame exposure, or a stage-1 run scoring above the baseline at the same cadence. |
| C51 | **Stage 2 shows a one-capture black full-client host mid-drag on the coarse synthetic drag — 2 of 3 stage-2 runs, 0 of 7 stage-1 and baseline runs** — chrome, footer and margins black while the inset page is complete at the new width | `SUPPORTED` | prod `s2b` `00ac32aa3115b455`, 25 px / 120 ms, cells `exp_e5a2a7` (frame f18, 1660x650 — the last step of the first grow) and `exp_11a2a3` (f14, 1560x650 — mid-grow); the diag build `exp_4b7464` and every stage-1 / baseline run of [[C50]] had none (top band worst ≤ 5 %). Traces `+timestamp`; frames kept in the run dirs | **Measured:** in each, one frame with the top band **100 %** true black, bottom 78–79 %, left 22 %, right 23 %, interior lit (GAPS 0): the full-client child's host draws nothing while the page host (a separate child, zpos 5) is intact and already at the new width. Looked at: `k5 f18`, `k9 f14`. Aligned to the trace by the driver's wall stamp against the first `SysCommand f002` (±150 ms): in the ~100 ms before each capture the root pass **stretched** the full-client child (`stretched (0,0)-(1610,650) -> (0,0)-(1635,650)`) and ~22 ms later CEF **re-created** it (`RELEASE` of the generation before, `CREATE` of the next, `retire_superseded_layers` of the current). T6 PASS in both runs; `macdrv_size_move_ended` 6 of 6 segments. **RESOLVED 2026-09-06 by [[C56]]: mechanism (a) is right in outcome but wrong in detail, and (b) is dead.** The frame is the child's OWN offscreen layer before its first drawable (S3, blue on the diag build), not the retired host's background; and a positive rendered colour cannot be a capture artifact. The original two candidates as written: **Inferred, not measured — two candidate mechanisms:** (a) a display gap: the stretched host is retired at its replacement's CREATE and the replacement has not presented yet, so the content view's black shows for a frame — but that retire-on-create sequence runs on stage 1 too, which never showed it, so the stretch (a transform on the host) would have to lengthen or expose the gap; (b) a capture artifact: `screencapture -l` missing a remote layer that carries a non-identity transform at the instant of the window server's commit, invisible to a user. Nothing here separates them; **T3's eyes do** (a black flash of the chrome during a hand drag = display), or a full-screen capture at the same instant. **Tracked in issue #12.** **REFUTED IN PART 2026-09-05 ([[C54]]): the frame is NOT stage-2-specific.** It occurs on the stage-1 daily driver (2 of 5 full-coverage runs, module hash verified, band signature identical to a tenth of a percent). This row's control — 0 of 7 stage-1 / baseline runs — was measured at `FRAMES=60`, the first quarter of the drag, and could not reach the frames where they occur. The MEASUREMENT of the two frames stands; the attribution to stage 2 does not. **Amended 2026-09-05 ([[C52]]): the rate is 2 of 7, not 2 of 3** — four further window-capture runs at this cadence produced none, so the 2-of-3 here is the small-sample high end and any power calculation must use ~29 %. The two frames stand; only the rate is revised. **Consequence:** the stage-2 module stays out of the daily driver until this is understood, and T2b / T3 read the **top** band too — a right-band-only reading would have filed this as a 23 % strip. **What would overturn it:** three further prod runs at 25 / 120 with none (chance at 2 of 3), or the same frame on stage 1. |
| C52 | **The stage-2 black frame did not recur in four more window-capture runs — 2 of 7 overall, not 2 of 3 — and the composited-display control cannot yet decide the mechanism** | `PARTIAL` | `scripts/issue12-capture-control.sh`, prod `s2b` `00ac32aa3115b455`, [[C50]]'s cadence unchanged (25 px / 120 ms, `PRESIZE=1000x650`, right +650 ×3, top −350), Steam store page; three window/screen pairs INTERLEAVED so machine drift hits both arms alike. Run `~/cs2-patch/issue12/20260905-163753`, frames kept. ⚠ **Cite this row by RUN DIR, not by cell id.** Its six cells were named `drag-t3-p<n>-<mode>` and [[C53]]'s battery reused those exact names, so `salvage-cells.sh` merged the two into one evidence dir each and the `exp_` ids now resolve to C53's runs. Nothing measured was lost — the frames, `bands.txt`, `probe.txt`, `stdout.txt` and the per-run `cell.txt` fingerprint all live in the run dir, which never collided — but the cell-store `config.json` for this row is C53's. GOTCHAS has the trap; the battery now carries its own timestamp in every row name. A fourth window row survives from an aborted first battery (`~/cs2-patch/issue12/20260905-162315/p1-window`): the harness died in its DIGEST after the row had captured and scored, so the frames are intact — see GOTCHAS on `local` under `set -u` | **Measured — top band ≥ 50 % true black (the [[C51]] signature): 0 of 3 window runs and 0 of 3 screen runs**, worst top band 4 % / 0 % / 0 % and 0 % / 0 % / 0 %. With the salvaged fourth row that is **0 of 4 window runs today against C51's 2 of 3 — 2 of 7 overall**. Every row caught the drag (19–23 distinct sizes), GAPS 0, interior-lum median 187 in BOTH arms (so the region captures are of Steam, not of the desktop), T6 PASS 6 of 6. One screen row is **VOID by the probe's own guard** (`overlaps=1/0` — a window over Steam at arm time), and its numbers match the other two anyway. **Right band ≥ 20 %:** window 2 / 0 / 0 (worst 40 / 18 / 17 %), screen 0 / 0 / 0 (worst 2 / 2 / 2 %). **Instrument drift, measured from window mode's own capture-size vs after-size:** the window grows **25–75 px (mean 28) during a single capture, on ~15 % of frames**. **Inferred, not measured — and one inference is REFUSED.** The screen arm's near-silent right band is **not** evidence that issue #7's strip is a capture artifact, and must not be read that way. Two things forbid it: (i) screen mode freezes its region at the rect read *before* the capture, so the ~88 ms capture crops off the 25–75 px the window grew — on exactly the growing frames the strip appears on; (ii) the strip was **seen by eye** (issue #7, 2026-09-03), so it is on the display whatever a region capture reports. The control stays valid for a WHOLE-HOST signature, which a 28 px right crop cannot hide — there was simply no such frame in either arm to compare. **What would overturn it:** the black frame recurring in a screen-mode run (→ display gap, decided); or ~10 screen runs with none while window runs keep producing it at ~29 % (→ artifact, p ≈ 0.03). Tracked in issue #12. |
| C53 | **Sampling the whole drag instead of its first quarter changes the picture: the black full-client frame recurs (2 of 10 window runs), it happens on SHRINKS as well as grows, and the growing-edge strip appears in composited-display captures too — but window-vs-screen at n=10 is still not significant** | `PARTIAL` | `scripts/issue12-capture-control.sh` `N=10 FRAMES=300`, prod `s2b` `00ac32aa3115b455`, [[C50]]'s cadence (25 px / 120 ms, `PRESIZE=1000x650`, right +650 ×3, top −350), Steam store page; ten window/screen pairs interleaved. Run `~/cs2-patch/issue12/20260905-180125`, cells `exp_8ba271` `exp_feaa98` `exp_13420d` `exp_182090` `exp_89c2a7` `exp_f2410a` `exp_004616` `exp_a032b7` `exp_b8873d` `exp_0d4209` `exp_766988` `exp_7ca668` `exp_fff81d` `exp_65ec9e` `exp_1de8d3` `exp_f4c093` `exp_817648` `exp_c9f620` `exp_91133b` `exp_e00eb8`, 6000 frames kept. **The change that mattered:** the synthetic drag lasts **35.4 s** (six `SC_SIZE` segments, first-to-last stamp) and `FRAMES=60` samples **8–9 s**, so every row through [[C52]] scored roughly its FIRST QUARTER and the `SYNTH_REPEAT=3` segments were never sampled at all | **Measured — top band ≥ 50 % true black: window 2 of 10 runs (3 frames), screen 0 of 10 (3 rows VOID on the overlap guard).** The three frames: `p1-window` f130 (1560x650) and f180 (1360x650), `p9-window` f8 (1385x650), all T=100 %. **Two of the three sit at frames 130 and 180 — a `FRAMES=60` run could never have sampled them**, which is why the same battery at 60 frames scored 0 of 4 four hours earlier. **f130 falls on a SHRINK** (1610 → 1560 → 1535), where C51's two frames were both grows. **Not significant:** Fisher two-tailed on 2/10 vs 0/10 is **p = 0.474** (per-frame 3/3000 vs 0/3000, p = 0.25); at this rate ~30 pairs would be needed for p < 0.05. **Right band ≥ 20 %, worst per run — window:** 23 · 78 · 100 · 100 · 24 · 44 · 35 · 46 · 78 · 40 %; **screen:** 25 · 6 · 4 · 16 · 11 · 2 · **100** · 2 · 12 · 22 %. **Measured, and it settles a question C52 left open: the strip IS in the composited display.** `p7-screen` f68–f71 read R = 100.0 % while the window grew 1135 → 1260, the same signature as `p3-window` f100–f103 (R = 100 %, 1485 → 1585). So a region capture can and does show the black growing edge, and C52's near-silent screen arm was its tracking lag attenuating the strip, not the strip being an artifact of `screencapture -l`. Independent of the eye observation in issue #7, which said the same thing. **Inferred, not measured:** that the top-band frame is display-real. 0 of 10 screen runs is suggestive and no more; the artifact hypothesis now has to be specific to the whole-host case, since `-R` demonstrably renders a 100 % black band elsewhere in the same battery. **What this does NOT license:** reading [[C50]]'s stage-1-vs-stage-2 comparison as a whole-drag result — it is a first-quarter comparison, internally consistent but quarter-scope, and no stage-1 or baseline run exists at `FRAMES=300`. **What would overturn it:** the frame appearing in a screen run; or ~30 pairs holding 20 % vs 0 %. Tracked in issue #12. |
| C54 | **Measured over the whole drag, stage 1 halves the strip and stage 2 makes it worse — and the black full-client frame is NOT stage-2-specific: it occurs on the stage-1 daily driver too** | `SUPPORTED` | `scripts/strip-module-ab.sh` `N=5 FRAMES=300`, window capture, [[C50]]'s cadence (25 px / 120 ms, `PRESIZE=1000x650`, right +650 ×3, top −350), Steam store page; five runs each of baseline `cd79fc463795939f`, stage 1 `2a251a4b2510fb84`, stage 2 prod `s2b` `00ac32aa3115b455`, **interleaved** b/s1/s2 so drift hits all three alike. Run `~/cs2-patch/strip-ab/20260905-195227`, 4500 frames kept; cells `exp_dbf64a` `exp_3e260d` `exp_ee2bb4` `exp_56e3f3` `exp_fce642` `exp_68c20e` `exp_84a0cf` `exp_28df7d` `exp_b4da9d` `exp_d70578` `exp_905505` `exp_8d005e` `exp_3bf91a` `exp_f7482a` `exp_3dd6ca`. Metric is `band-counts.py`'s growing-frame right-band mean, the same code C50 used, so the two rows compare | **Measured — growing-frame right band, per-run means:** baseline 6.3 · 43.3 · 6.8 · 6.3 · 4.3 % (median **6.3**), stage 1 3.0 · 3.1 · 3.4 · 3.0 · 3.4 % (median **3.1**), stage 2 4.8 · 4.7 · 4.5 · 5.7 · 3.2 % (median **4.7**). **Stage 1 vs baseline: complete separation — every stage-1 run below every baseline run, Mann-Whitney p = 0.0079.** **Stage 2 vs stage 1: p = 0.0317, stage 2 is WORSE.** Stage 2 vs baseline: p = 0.0952, not separable. Stage 1 is also far tighter (3.0–3.4 against baseline's 4.3–43.3). **Measured — the black full-client frame (top band ≥ 50 %): stage 1 2 of 5, stage 2 1 of 5, baseline 0 of 5.** The three frames are `r1-stage1` f10 (1460x650), `r4-stage1` f126 (1435x650), `r1-s2b` f121 (1510x650); module hashes verified from each row's own log (`2a251a4b2510fb84` twice, `00ac32aa3115b455` once). **The band signature is identical to [[C51]]'s two frames to a tenth of a percent — L 22.0 · R 23.0–23.1 · T 100.0 · B 78.5, interior lit (GAPS 0) — at five different window widths (1435 / 1460 / 1510 / 1560 / 1660), so it is one geometric event, not a coincidence of thresholds.** **This refutes C51's premise by existence, not by significance.** "Stage 2 shows it, 0 of 7 stage-1 / baseline runs do" was measured at `FRAMES=60` — the first quarter of the drag ([[C53]]) — and two of these three frames sit past frame 120, where that control could not look. Two verified stage-1 occurrences are enough to retire "stage-2-specific"; no p-value is needed for an existence claim. **Inferred, not measured — and ANSWERED 2026-09-06 by [[C55]]: stage 1 did NOT introduce it.** A pre-stage-1 baseline run produced the frame with the identical signature, so the defect is architectural. The original note stands as written: baseline's 0 of 5 against stage 1's 2 of 5 was Fisher **p = 0.444** — nowhere near enough, and the pre-stage-1 baseline has never been run at full coverage for more than five drags. **Consequence:** the case for promoting stage 2 is gone — it does not beat the installed driver on the strip, it measurably loses to it, and it does not avoid the black frame. And the black frame is a defect of the **shipping** daily driver, not a blocker peculiar to stage 2. **What would overturn it:** a stage-2 build scoring below stage 1 over ≥ 5 full-coverage runs; or ~15 baseline runs at full coverage with no black frame, which would put its origin on stage 1. Tracked in issue #12. |
| C55 | **The black full-client frame occurs on the PRE-STAGE-1 BASELINE — it is architectural, and neither stage introduced it** | `SUPPORTED` | `scripts/strip-module-ab.sh` `MODULES="baseline s1diag" N=10 FRAMES=300`, window capture, [[C50]]'s cadence, Steam store page; run `~/cs2-patch/strip-ab/20260906-005246`. **Only 5 of 20 rows completed** — a ~4 minute VPN reconnect took the network down at 01:06 and `cell-fingerprint.sh --strict` correctly refused every remaining row (`FATAL network DOWN`); because a refused row aborts in ~4 s against a real row's 3.5 min, the queue drained in 4 minutes. GOTCHAS has it; the battery now blocks on connectivity per row. Valid cells: baseline `exp_9174b4` `exp_ee00ed` `exp_284c55`, stage-1 diag `exp_bf9252` `exp_bb718c`. The 15 refused rows are fingerprint-only cells with no measurement | **Measured — `r2-baseline` (`exp_ee00ed`) produced the black frame on module `cd79fc463795939f`, the pre-stage-1 baseline**, hash read back from the row's own log. Frame f44, 1385x650, **L 22.0 · R 22.0 · T 100.0 · B 78.5**, interior lit (GAPS 0) — the [[C51]] / [[C54]] signature to a tenth of a percent, now at a sixth distinct window width. Baseline is therefore **1 of 3** here, against [[C54]]'s 0 of 5 at the same coverage. Also measured: baseline growing-frame right-band means 6.1 % (n=3), consistent with C54's 6.3 % median; the stage-1 DIAG build ran 5.0 % and 39.3 % (n=2) and produced **no** top-band frame in 2 runs, so the colour attribution is still unanswered. **This settles what [[C54]] left unclaimed.** C54 inferred nothing from baseline 0 of 5 (Fisher p = 0.444) and named "~15 baseline runs with no black frame" as what would put the origin on stage 1. The opposite arrived first: **one verified baseline occurrence removes stage 1 as the origin**, by the same existence argument that removed "stage-2-specific" — a single run with the module hash checked and the signature matching cannot be argued away by a p-value. **Consequence:** the frame is architectural, like [[C35]]'s strip, and belongs to the cross-process hosting layer rather than to either stage. Issue #12 is a defect of the *design*, not of a build. **Inferred, not measured:** its rate per module — 1/3, 2/5 and 5/22 are all small and mutually consistent, and nothing here separates them. **Still unanswered:** what the black region is attributed to; the diag arm needs ~10 runs to catch one and got 2. **What would overturn it:** a re-read of `exp_ee00ed`'s log showing a different module hash. Tracked in issue #12. |
| C56 | **The black full-client frame is S3 — the child's OWN offscreen layer before its first drawable — and it is therefore real, not a capture artifact** | `SUPPORTED` | `scripts/strip-module-ab.sh` `MODULES="baseline s1diag" N=10 FRAMES=300`, window capture, [[C50]]'s cadence, Steam store page; run `~/cs2-patch/strip-ab/20260906-091345`, all 20 rows valid (the connectivity gate added after [[C55]] held). Ten runs on the stage-1 **diag** build `50fdfe79898dac36`, which paints the host backgrounds — green S1 (a host reframed larger than its content), blue S3 (the child's own layer before its first drawable), magenta S2 (the deferred create path), black-with-no-colour S4 (nothing hosting the area). Definitions from `scripts/diag-colours-patch.py:64-67`, not from memory. Key cell `exp_64c61c` | **Measured — `r7-s1diag` frame f10 (1510x650): top band `T=0.0` black and `Tblue=100.0`.** Nothing black anywhere in that frame (L R T B all 0.0); the band is *entirely blue*. Module hash read back from the row's own log: `50fdfe79898dac36`. One occurrence in 10 runs, and it is the ONLY diag run at full coverage ever taken — every earlier diag run was `FRAMES=60` and could not reach it ([[C53]]). A scan of every diag `bands.txt` on disk returns exactly this one frame. **Two things follow, and the second is the one that closes issue #12's original question.** (1) **The mechanism is S3**: the child's own offscreen layer is up and composited but has not presented its first drawable, so its background shows — black on a prod build, blue here. That is consistent with [[C51]]'s trace, which put a CEF re-create ~80 ms before the capture: re-create → new layer → no drawable yet. It is NOT the host-background/stretch-retire race C51 assumed. (2) **It cannot be a capture artifact.** A `screencapture -l` that failed to see a transformed remote layer would show an ABSENCE; blue is a positive, rendered background colour composited into the frame. **The display-gap vs capture-artifact question is answered: display gap.** [[C52]]'s and [[C53]]'s inconclusive window-vs-screen arms are moot. **⚠ The scorer was blind to this by construction and reported `0 of 10`.** The criterion was "top band ≥ 50 % TRUE BLACK", on a build whose whole purpose is to paint that region a colour. Fixed: the battery now counts a top band ≥ 50 % black **or** ≥ 50 % of any diagnostic colour; re-scored, the arm reads 1 of 10. Same family as the [[C49]] T6 cut and the [[C53]] sampling window — a criterion that cannot fire on the instrument it is pointed at. **Inferred, not measured:** the rate per module. Across all coverage: baseline 1 of 18, stage 1 2 of 5, stage-1 diag 1 of 10, stage 2 5 of 22 — small, mutually consistent, and nothing separates them, which is what [[C55]]'s "architectural" reading already implies. **What would overturn it:** a second diag occurrence carrying green or magenta instead of blue. Tracked in issue #12; the fix design is issue #13. |
| C57 | **The host layer normally trails a growing window by 50-100 px, and in 1 run in 20 it does not track a whole grow segment at all — sitting at the pre-drag width for 7.2 s while the window grows 650 px** | `SUPPORTED` | Re-analysis of traces already on disk; **no new runs**. `scripts/host-frame-stalls.py` over all **56 completed runs** in `~/cs2-patch/strip-ab` (26 baseline, 13 stage-1 diag, 12 stage 1, 5 stage 2 `s2b`) — 42 re-analysed, then a further 14-row `MODULES="baseline stage1" N=7` battery run 2026-09-06 14:07-14:56 specifically to test the rate, every one a `FRAMES=300` whole-drag row at [[C50]]'s cadence (25 px / 120 ms, `PRESIZE=1000x650`, right +650 x3, top -350) with `TRACE=+err,+macdrv,+cursor,+timestamp`. Metric: the widest single install-to-install GROW of one child's hosted frame, in time order, excluding a host's first `0 -> full size` install (the create path) and jumps completing under 1 s. The four runs that show the stall are cells **exp_56e3f3** (`drag-s1-20260905-195227-r2-baseline`, baseline `cd79fc463795939f`), **exp_bb718c** (`drag-t0-20260906-005246-r2-s1diag`, diag `50fdfe79898dac36`), **exp_51a0e7** and **exp_740386** (`…-140732-r2-stage1` and `-r5-stage1`, both **`2a251a4b2510fb84` — the INSTALLED daily driver**, hash read back from each row's own log) | **Measured — the distribution is bimodal with nothing in between.** 52 runs track to within **50 px (20), 75 px (27) or 100 px (5)**; 4 runs jump **650 px in a single install, and all four take 7.2 s to the millisecond**. No run lies between 100 and 650 px, so this is a distinct failure mode and not the tail of the ordinary lag. **Rate 4 of 56 (7%).** **It occurs on the INSTALLED DAILY DRIVER**, twice — so this is a defect a user meets, not one confined to a diagnostic build. **Corroborated independently from the pixels, which were scored without reference to the trace:** on both runs the captured frames give a CONSTANT solved content width across the episode — dark% = (W - x)/W yields a constant x on all four — **1006, 1006, 1007, 1031 and 1006 px** over 5, 10, 17, 12 and 16 consecutive frames while the window grows to 1585-1685, each matching its trace's stalled install width to within 4 px — matching the trace's stalled install width to within a pixel. The exposed region carries **0.0 % of every diagnostic colour** on the diag arm, i.e. it is [[C39]]'s S4, at up to 40 % of the window for ~1.9 s. Also measured, on the same traces: after retiring a superseded layer the host takes a median **100 ms** to install the replacement's frame (`scripts/layer-gap.py`, n=260 on one diag run). **This names [[C54]]'s and [[C55]]'s unexplained outliers.** C54 recorded baseline growing-frame means of 6.3 / **43.3** / 6.8 / 6.3 / 4.3 % and C55 a diag arm of 5.0 / **39.3** %, both treated as run-to-run spread; the high runs are exactly the stall runs, and the spread was a mechanism. The dedicated battery reproduces that correspondence with **complete separation**: its fourteen growing-frame means are 1.5 · 3.0 · 3.2 · 3.2 · 3.8 · 5.3 · 5.3 · 5.5 · 6.4 · 7.7 · 8.1 · 8.7, then **38.2 and 40.8** — and the two high runs are the two stalls. **Inferred, not measured:** what causes a segment to be skipped. The trace shows the host was never asked — `update_remote_layer_frame_for` does not run — but nothing here separates a child that stopped posting from a host that dropped the messages. Also unclaimed: whether the rate differs by module — baseline 1 of 26, stage 1 **2 of 12**, diag 1 of 13, stage 2 0 of 5; baseline against stage 1 is Fisher **p = 0.229**, nowhere near separable, and [[C55]]'s architectural reading predicts it does not differ. ⚠ **The detector is easy to get wrong in three ways and each was made here first:** grouping installs by child+context scores 0 of 42 (every resize step mints a new context, so the width progression lives across them), pooling all children scores 86 % (one window's children legitimately differ in size), and counting the create path as a stall inflates every run. **What would overturn it:** ~40 further whole-drag runs with no 650 px install jump, which would put the four occurrences down to something outside the drag. (The first 14-row test of exactly this returned two more, at the same 7.2 s.) Tracked in issue #7. |
| C58 | **The S3 gap is a SINGLE-FRAME flash, not a lingering hole — under 25 ms, but covering 13-53 % of the recorded area** | `SUPPORTED` | `scripts/video-gap-battery.sh` N=12, role t0 (stage 1 + diag colours, `50fdfe79898dac36`), 2026-09-06 16:28-17:04, **all 12 rows valid** (`locked=no/no overlaps=0/0` on every row). Each row records the whole drag with `screencapture -v` over a rect covering the window's entire travel and scores every video frame with `scripts/video-blue.swift` at `darkboxes.swift`'s thresholds unchanged; episodes are consecutive-frame runs, timed off presentation timestamps. Achieved cadence **25.0 ms median** (range 16.7-25.0), i.e. **4.5x** the frame probe's ~113 ms — stated in every row because it is the resolution floor under every duration here. Key cells **exp_8596e7** (r4) and **exp_09db5d** (r9) | **Measured — blue (S3): 4 episodes over 12 runs (0.3 per run), EVERY ONE a single frame.** Median 17 ms, p90 25 ms, max 25 ms — so the true duration is bounded above by one inter-frame gap and is at most ~25 ms, 1-2 frames at 60 Hz. **It is large while it lasts:** r4 f906 13.17 % (206,213 px), r9 f1013 **52.74 %** (837,782 px), f1203 44.79 %, f1512 40.78 % of an 1800x870 recording — against a window of at most 1660x800, so roughly half the client area. The frames either side read **0.0000 %**: `0 -> 52.7 -> 0` at 16-25 ms spacing, three times independently. **This answers what [[C56]] could not.** C56 established the mechanism from one captured frame and bounded the duration only to 'under ~226 ms'; the flash is an order of magnitude shorter than that ceiling and does not persist at all. **Also measured, first duration figures for the other two sources:** green (S1, the growing-edge strip) **51.2 episodes per run, median 17 ms, max 275 ms**, peak 0.04 % of the recording — frequent, single-frame, and tiny; magenta (S2, the deferred create path) 24.2 episodes per run, median 92 ms but **p90 3867 ms and max 9933 ms**, peak 0.03 %. ⚠ **Investigated 2026-09-06 and it is the mechanism WORKING, not a defect — see [[C59]].** The first reading of this row said the persistence was more than the create path's own comment leads you to expect; that was a misreading of the comment, corrected there. **Inferred, not measured:** whether a single-frame flash of half the client area is perceptible. That is T3, and it is now a much narrower question — the thing to look for is a rare flash, not a black region that lingers, and at 0.3 per drag it needs several drags to meet one. **Consequence for issue #13:** a fix aimed at shortening the gap has almost nothing to shorten. Read with [[C57]]'s note that the pre-drawable state tiles the whole drag, the design question is why the covering fails on ~1 frame in 4,500, not how long it is uncovered. **What would overturn it:** a blue episode spanning two or more consecutive video frames, which would make it a duration rather than a flash. ⚠ **Corrected 2026-09-07 (check-it platform-facts lens on `docs/plans/s3-pre-drawable-gap.md`):** the drag display is the 1920×1080 **main panel at 120 Hz** (`system_profiler`), not 60 Hz. A single captured frame at a 16.7–25 ms cadence bounds the flash **below two intervals (< 50 ms, 1–5 refreshes)**, not ≤ 25 ms; and a periodic sampler catches a one-refresh (8.3 ms) flash with p ≈ ⅓, so **0.3 per run is a lower bound** on the rate. The '1-2 frames at 60 Hz' earlier in this row is the error. Measured numbers unchanged. |
| C59 | **The S2 magenta is a ONE-DEVICE-PIXEL seam at the window's right edge, and its persistence is the deferred background working as designed — not a defect** | `SUPPORTED` | `scripts/video-blue.swift --where magenta` (locate rather than count, added for this) over the recordings of [[C58]]'s battery, stage-1 diag `50fdfe79898dac36`; run `r1-s1diag`, 701 magenta frames of 1805. Control for the scorer: the VOID lock-screen recording of the same day, which contains no Steam at all, scores **0 magenta in 1073 frames** — so magenta is not an encoder artifact | **Measured — every magenta region in all 701 frames is exactly ONE pixel wide** (`widths seen: [1]`), a single column 380–500 px tall whose x tracks the window's growing right edge (1436 → 1510 → 1560 → 1610 → 1660 → 1686). The long episodes sit at a FIXED x (1686, 1660) — the right edge while the window rests at its final width — and the 9933 ms episode is frames f1499–f1804 at x=1686, i.e. the window sitting still at the end of the recording. **That is what the code says it is for:** *"The background covers what the snap below cannot — an odd Win32 rect in a whole-point slot leaves a sliver uncovered, showing white … transparent until only the seam is left to reveal"* (`cocoa_window.m`, the deferred-background block in `addCALayerHostViewWithContextId:frame:`). A seam persists exactly as long as the window sits at that size, so seconds of it is the design, and on a prod build that column carries the unmodified `kCGColorBlack` — the diag patch recolours that one assignment and nothing else. **⚠ Corrects [[C58]]**, which reported the persistence as something the comment does not lead you to expect. It does; I had read only the first clause. **The finding that survives is INSTRUMENTAL and it is the useful one:** the window capture scores **0 magenta in 300 frames** on this same build across every diag run, because a 1-device-pixel seam does not survive it — which `scripts/pixel-probe.swift`'s own header has said since 2026-08-31. **So `CAPTURE=video` sees 1-px edge features that every instrument before it was structurally blind to**, and that is a capability worth more than the magenta was. **Inferred, not measured:** whether the black seam is noticeable in use. It is 1 device pixel against Steam's dark chrome, so probably not, but nothing here measures perception. **What would overturn it:** a magenta region wider than one pixel, or one not at a window edge. |
| C60 | **The stall is on the CHILD, not the host: during it the child stops publishing swapchains while the window keeps resizing normally — and it happens only on GROWS** | `SUPPORTED` | `scripts/stall-window.py` over the four stall runs of [[C57]] (`exp_56e3f3` baseline, `exp_bb718c` diag, plus `…-140732-r2-stage1` and `-r5-stage1` on the installed driver `2a251a4b2510fb84`), counting each side's trace events inside the stall window against the preceding 10 s. Windows taken from `host-frame-stalls.py`, not by eye | **Measured — the counts inside the stall are IDENTICAL in all four runs**, which is itself the finding: acquire (child publishes) **2**, present (child draws) **4**, swapchain request (child asks) **2**, install (host places) **3**, retire (host got the CREATE message) **2** — against 72–88 / 144–176 / 72–88 / 70–88 / 72–88 in the preceding 10 s. So the child drops to ~3 % of its rate. **Meanwhile the window keeps resizing at full rate:** `window_frame_changed` **26** — exactly one full drag segment of 26 steps — and `sync_window_position` **312** in every run, 43/s, the same rate as outside the stall. **That last number is the internal control and is what makes this a measurement:** the trace is not blocked and the host thread is not wedged, so the absence of the child's events is real and not an instrument artifact. **Measured — the asymmetry: 4 grow stalls and 0 shrink stalls over 56 runs** (jumps > 300 px taking 1–30 s; the 1–30 s bound matters, because every run also contains a ~100 s shrink 'jump' that is just the window sitting idle between its setup and its drag, and counting those reports a shrink stall in 56 of 56). Position: 3 of 4 stall on the LAST grow, 1 on the middle grow; never on a shrink. **Consequence:** the fix for [[C57]] is not in the host's placement path. The host had nothing to install because nothing was published — `retire_superseded_layers` fired twice, so it processed every CREATE it was sent. This is DXMT/CEF ceasing to re-create its swapchain for one grow segment. **Inferred, not measured:** why. Nothing here says whether the child is blocked, throttled, or simply not asked by CEF; the trace only carries winemac's side of the boundary. **What would overturn it:** a stall in which `sync_window_position` also stops, which would make the whole trace suspect rather than the child. Tracked in issue #7. |
| C61 | **Under a LIVE DRAG the content view's own layer is exposed at the growing edge as a column an integer number of drag steps wide — 1 to 4 x 25 px, full window height, ~35 frames per drag** | `PARTIAL` | `scripts/video-gap-battery.sh N=10 MOD=t0b`, 2026-09-06 23:22-23:52, all 10 rows valid. Module **`38b52d6b3971d78b`** — [[C42]]'s build: the content view's GDI blit suppressed and its own layer painted **cyan**. Video capture at a 25 ms median cadence, scored by `video-blue.swift --where cyan` (locate, not count). Cells incl. **exp_c72a8f** (r1) **exp_1eba59** (r9) | **Measured — in 8 of 10 runs the cyan appears as a NARROW, FULL-WINDOW-HEIGHT column at the growing edge**, 30-41 frames per run, y 180..828 (h 649 = the window). **Its width is quantised to the drag step:** the histogram is 22-26 px (187 frames), 48-49 (56), 73-75 (9), 99 (6) — i.e. 1, 2, 3 or 4 steps of `SYNTH_PX=25`, median **25 px**. So the exposed area is exactly the ground the window gained since the layer last kept up. **This reaches what [[C42]] could not:** C42 ran t0b under CHURN, which never enters win32u's SC_SIZE loop ([[C43]]), and scored it with the frame probe, which cannot see a single-frame event. Under a real drag the content view's layer is a frequent source of the strip. **⚠ `PARTIAL`, and the missing half is the denominator.** This counts frames where the layer IS exposed; it does not establish what share of ALL exposure frames that is, because a black-region count cannot be shape-filtered the way cyan can — black is everywhere (page artwork, desktop) and its bounding box spans the frame. So C42's 3-of-15 split is NOT superseded, only complemented. **⚠ Two runs are confounded and the reason matters:** r4 and r5 scored 1850 and 1803 cyan frames against the others' 30-41, from a **wide short block** (489x305 at x 1253..1741, present from frame 0) — Steam's own artwork. C42 chose cyan as *"a fourth diagnostic colour the store page cannot contain"*; on the page these runs loaded, it can. The shape filter (height >= 600 AND width <= 200) separates them cleanly, and colour alone never could. **Inferred, not measured:** that the same column is what a prod build renders black. Nothing here paints a prod build. **What would overturn it:** the same battery on a page with no cyan artwork showing the column at a width unrelated to `SYNTH_PX`. |
...
| C63 | **A `--drop` is SOUND: an `opaque = YES` CAMetalLayer with no background that has never presented composites as NOTHING through a `CALayerHost`, cross-process** — so the S3 plan's candidate A is not inert, and A/D can be built on it | `SUPPORTED` | **S0, measured directly rather than inferred.** `scripts/calayerhost-probe.m` (new), 2026-09-07: three arms hosted SIDE BY SIDE over one saturated-green backdrop and read from **one** capture — `black` (backgroundColor black, as wine does today) is the **control**, `drop` (no background, `opaque = YES`) is candidate A, `nonopq` (no background, `opaque = NO`) is A's fallback form. Layer properties match DXMT's rewrite (`winemetal_unix.c:1517-1529`): `framebufferOnly = NO`, `contentsScale`, an explicit `drawableSize`. Two topologies: same-process, and `--xproc`, where a **separate process** publishes the `CAContext` and the prober hosts it — wine's actual child/owner topology. No Metal rendering and no drawable is ever acquired: "published but never presented" IS the state under test | **Measured. same-process 4 of 4 runs; cross-process 4 of 5 valid runs, 1 refused. Every valid run agrees: control `0,0,0` (black) and `drop` `0,249,0` (the green backdrop straight through), in the SAME frame.** `nonopq` reads the same as `drop`. ⚠ **The design point that makes this readable at all:** "the layer contributed nothing" and "the layer had not committed yet" are the SAME green reading, so measuring arms in separate captures cannot tell them apart — the first version did exactly that and its control came back green on 2 runs in 3, intermittently blind. Hosting all three in one frame makes the control's blackness a property of the very frame the others are read from; the capture is retried until the control is black and the run is **VOID** if it never is (1 of 5 cross-process runs, which reported all three arms green and named the cause). **Inferred, not measured:** that the same rule holds through the full DXMT pipeline under a live drag — the plan's in-situ S0 (A-drop + cyan content view, module `d79c80d951b3a5e9`) is the confirmation and is separate from this. **What would overturn it:** a valid run where `drop` reads black while the control also reads black |
| C64 | ~~**The in-situ S0 arm is VOID: the Steam LIBRARY page supplies both diagnostic colours**~~ — **the page was never the problem; the SCORER was** | `RETRACTED` | **S0 in situ, both arms.** `scripts/video-gap-battery.sh N=12 STEAM_PAGE=steam://open/games`, 2026-09-07: arm A = `d79c80d951b3a5e9` (candidate A `--drop` + `--cyan`, `--noblue`), arm B = the plan's own mutant `e700ac8fdfef0e80` (`:4339` restored, blue ON, `--cyan`). 12 valid rows each, video, identical achieved cadence (16.7 ms median, no spread) | **Measured and REFUSED. cyan 542 (45.2/run) vs 543 (45.2/run)** — one episode apart across builds whose only difference is whether the child's layer carries a black background, so the cyan is the PAGE, not the surface. **The plan's mutant fails in the diagnostic direction too:** blue was 19 episodes on the arm carrying NO blue patch and 12 on the arm that has one — backwards, so blue is page content as well. Episode durations say the same thing louder: cyan p90 1350 ms and max 16.6 s, where a real S3 exposure is a single frame (C58). **Root cause: the page knob was used to dodge C61's cyan store artwork and walked into a page that supplies cyan AND blue.** Third of the family (GOTCHAS *A diagnostic colour is only safe until the page under test contains it*). **What survives:** nothing about A from this run — but S0's MECHANISM is not affected, it is answered by [[C63]]'s direct probe, which carries its own control. **What it takes to redo:** a page supplying neither colour, or the shape gate declared before the run (full-client = cyan bbox w >= 50 % of window width AND h >= 50 % of its height), plus this same mutant arm as the control that caught it. **⚠ AMENDED 2026-09-08 — RETRACTED, and the retraction is of the INFERENCE, not the run** ([[C65]]). The run is sound: 24 valid rows, guards clean, one cadence. What was wrong is the count it was read through. `video-gap-battery.sh` scores an episode at `fraction > 0` for every colour but black, and re-scored here **the median cyan frame among those 542 carries ONE PIXEL, at (30,479), in nearly every frame of nearly every run**. 542 against 543 compared a stray pixel with itself; nothing about the page was ever measured. The same reading destroys the blue half: blue's `>0` frames are single pixels too, and at any real bar **arm A (the `--noblue` arm) has ZERO blue frames in 12 runs while arm B has 4** — the mutant fires in the correct direction, so "blue is page content as well" is refuted by this run's own recordings. **What survives:** the measurements, and the discipline that caught it — the mutant arm. **What is withdrawn:** both inferences, and with them the claim that S0 in situ still needs redoing. It does not; it is answered by [[C65]] off these very recordings. **The lesson is NOT "pick a better page"** — it is that presence is the wrong test for a colour whose defect has a SIZE. `scripts/full-client-episodes.py` is that test. Cells: **exp_66e91e** **exp_9ecac7** **exp_7e3ba8** **exp_5886dd** **exp_92f19f** **exp_aa547d** **exp_995321** **exp_958338** **exp_6a0660** **exp_2eecf0** **exp_cc0946** **exp_a86345** **exp_f7a9b6** **exp_b8f4a2** **exp_69f99c** **exp_e96951** **exp_f179db** **exp_1f7380** **exp_02d65c** **exp_ec136f** **exp_145364** **exp_024f02** **exp_2bcda2** **exp_a85142** |
| C65 | **In situ, through the real DXMT pipeline, candidate A's `--drop` RELOCATES the S3 flash rather than removing it — the identical full-client region reads CYAN (the content view beneath) where the black background reads BLUE, and reads black on neither arm** — so the plan's S0 outcome 1 is confirmed in situ and **S4 is measured: A alone is not a fix** | `SUPPORTED` | **S0 in situ + S4, re-scored off [[C64]]'s own recordings** — no new run. `scripts/full-client-episodes.py` (new), which replaces the battery's `fraction > 0` tally with a SIZE test: a frame is full-client for a colour when that colour covers >= 0.80 x the window's **initial** area (`video-rect.txt`, a size the client never falls below). The bar is not tuned — swept over these 24 runs the counts are **flat from 0.65 to 0.92**, and 0.80 is the middle of that plateau; below 0.65 the [[C57]]/[[C60]] stall bleeds in, at 1.00 the bar passes the initial area and starts dropping flashes that happened before the window grew. Arms: **`d79c80d951b3a5e9`** (candidate A `--drop`, `--noblue`, cyan content view) and **`e700ac8fdfef0e80`** (the plan's mutant: `:4339` restored, blue ON, cyan content view), 12 valid rows each, video at a 16.7 ms cadence | **Measured, and the separation is complete. Arm A: cyan 2 episodes in 2 of 12 runs, blue 0, black 0. Arm B: blue 4 in 2 of 12, cyan 0, black 0.** Every episode is ONE frame. The regions coincide to within 4 pixels: arm A r1 f324 cyan `x 0..1034, y 180..828`, **671,660 px**; arm B r5 f329 blue `x 0..1034, y 180..828`, **671,656 px** — two modules, two runs, two colours, one surface. Peaks 93-155 % of the initial window area, i.e. the whole client. **`black` is 0 on both arms, which refutes "A-drop is inert" in situ:** were the layer still opaque, arm A's flash would read black, and it reads the content view. Two other phenomena share these recordings and neither reaches the bar — the [[C61]] growing-edge column (16,171 px = 2.2 % of the window, 26 distinct columns inside a 1005 px bbox, which is why the gate counts PIXELS and not the bounding box) and the [[C57]]/[[C60]] stall (28-36 consecutive frames growing monotonically, plateauing at 51-60 %), the latter in 1 of 12 runs on **each** arm. **Inferred, not measured:** that D's hold removes what A alone relocates — that is the interleaved S3/S4 battery, running separately. **⚠ Two limits, stated because the plan's own rules name them:** the arms were run **sequentially, not interleaved** (19:38 and 21:22 on 2026-09-07), so the between-arm RATE comparison (0.17 vs 0.33 per drag) is weak — the within-arm attribution, which is what this row claims, is not affected; and n = 12 per arm with 2 and 4 events is small. **What would overturn it:** a valid arm-A run whose full-client region reads black, or one where the mutant's blue region does not coincide with arm A's cyan region. Cells: the 24 of [[C64]] |
| C66 | **Candidate D shows ZERO full-client episodes where the interleaved A-alone control shows four — but the run was stopped at 10 of the planned 15 rows, so the plan's pre-registered closure bar is NOT met** | `PARTIAL` | **S3, interleaved with S4's control arm**, `scripts/video-gap-battery.sh MODS="D-diag s0-adrop-cyan" N=15 STEAM_PAGE=steam://open/games`, 2026-09-08 09:09-09:4x, **stopped by hand after 10 full rounds** (the machine was needed). Arms **`6385a7ffb1cbb408`** (candidate D + `--noblue --cyan`, nested branch `d` `72184cd`) and **`d79c80d951b3a5e9`** (candidate A alone + `--noblue --cyan`), one row of each per round — the first battery in this thread to interleave, using the new `MODS`. Scored by `scripts/full-client-episodes.py` at **BAR = 0.80**, committed *before* the run (see [[C65]]) | **Measured. D-diag: 0 full-client episodes of ANY colour in 10 valid runs. A alone: 4 cyan episodes in 4 of 10 valid runs (0.40 per drag), every one a single frame at 142-157 % of the initial window area.** Both arms delivered 10 valid rows, 150 children and ~1,600 generations each — the interleave held. ⚠ **This is NOT closure and must not be reported as it.** The plan fixes the bar in advance at 0 episodes over **n ≥ 15**, which is exact Poisson P(0) = 0.0067 against the pooled 0.333/drag baseline — the only n that clears 0.007. At n = 10 the same test gives **P(0) = 0.0357**; against this battery's own control rate (0.40/drag) **0.0183**; Fisher exact on runs-carrying-an-episode (0/10 vs 4/10) **p = 0.0433**. Suggestive, three to six times the bar. **Five more rounds settle it and nothing else needs redoing** — the arms, the page, the module digests and the gate are all fixed and recorded. **Inferred, not measured:** that D's zero means the predecessor is covering the window rather than the flash having moved somewhere the gate cannot see; a covered region shows the predecessor's own content, which is indistinguishable from correct rendering to this scorer and is exactly what **T3/S5 (human)** exists to judge — especially on shrinks. ⚠ The A arm's rate here (0.40/drag) is higher than the same module's rate last night (0.17/drag, [[C65]]); n is 10 and 12, so this is small-number variation and neither figure should be quoted as *the* rate. Pooled over both, A alone is 6 episodes in 22 drags. **What would overturn it:** a full-client episode on any D row |
| C67 | **Candidate D's hold is bounded at two hosts per child — 1,611 creates over 10 runs, max live 2, never 3 — and it also eliminates a housekeeping fault the current code has: 126 retired-but-never-released hosts become 0** | `PARTIAL` | **S7's first half only.** `scripts/live-hosts.py --max 2` replayed over the same interleaved battery as [[C66]]: 10 D rows (`6385a7ffb1cbb408`) against 10 A rows (`d79c80d951b3a5e9`) as the control. The gate exits non-zero if any child ever exceeds 2 | **Measured, and the DWELL column is what separates the arms** (max alone cannot — the plan's own instrument note says so, and both arms max at 2). **D: 150 children, 1,611 creates, retires 0, releases-that-freed 1,321, max live 2, dwell at ≥ 2 hosts 2,516 s of 2,525 s = 99.7 %. A: 150 children, 1,598 creates, retires 1,448, releases-that-freed 0, max live 2, dwell 0.4 s of 2,553 s = 0.0 %.** So the hold is real, it is doing precisely what § 4.1 specified, and it never reaches three. Visible transition by transition in `--dump`: `release(N−1) → create(N+1) → release(N) → create(N+2)`, the count oscillating 28↔29. **The unlooked-for result: `retired-but-never-released` falls from 140 to 0.** Under today's code the owner retires at CREATE and the child's later RELEASE finds an untracked id (§ 2.5 measured 271 of 271); under D nothing is retired, so every release does real work. **⚠ The cost, stated as a number rather than left implicit: D roughly DOUBLES the resident host count** — 290 hosts still tracked at the end against A's 150, and **140 of 150 children end holding 2** (A: 0 of 150), so a child's last predecessor is not released while the child lives. **That is exactly S7's second half and it is UNRUN**, along with the `--norelease` mutant (which under D has no other exit and must grow past 2) and `boot-verify.sh`. **This row is therefore `PARTIAL`, not "S7 green".** ⚠ Measured under a live drag, not the `shimmer-probe.sh churn` the plan's row names; the create count (1,611) far exceeds its ≥ 300 but the path is different. **What would overturn it:** any child reaching 3, or a terminal child whose held pair never drops |
...
| exp_8ba271 | 2026-09-05 18:03 | `drag-t3-p1-window` | 0 | 0 | 0 | rendered | candidate |
| exp_feaa98 | 2026-09-05 18:06 | `drag-t3-p1-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_13420d | 2026-09-05 18:09 | `drag-t3-p2-window` | 0 | 0 | 0 | rendered | candidate |
| exp_182090 | 2026-09-05 18:13 | `drag-t3-p2-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_89c2a7 | 2026-09-05 18:16 | `drag-t3-p3-window` | 0 | 0 | 0 | rendered | candidate |
| exp_f2410a | 2026-09-05 18:19 | `drag-t3-p3-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_004616 | 2026-09-05 18:23 | `drag-t3-p4-window` | 0 | 0 | 0 | rendered | candidate |
| exp_a032b7 | 2026-09-05 18:26 | `drag-t3-p4-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_b8873d | 2026-09-05 18:30 | `drag-t3-p5-window` | 0 | 0 | 0 | rendered | candidate |
| exp_0d4209 | 2026-09-05 18:33 | `drag-t3-p5-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_766988 | 2026-09-05 18:36 | `drag-t3-p6-window` | 0 | 0 | 0 | rendered | candidate |
| exp_7ca668 | 2026-09-05 18:40 | `drag-t3-p6-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_fff81d | 2026-09-05 18:43 | `drag-t3-p7-window` | 0 | 0 | 0 | rendered | candidate |
| exp_65ec9e | 2026-09-05 18:46 | `drag-t3-p7-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_1de8d3 | 2026-09-05 18:50 | `drag-t3-p8-window` | 0 | 0 | 0 | rendered | candidate |
| exp_f4c093 | 2026-09-05 18:53 | `drag-t3-p8-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_817648 | 2026-09-05 18:57 | `drag-t3-p9-window` | 0 | 0 | 0 | rendered | candidate |
| exp_c9f620 | 2026-09-05 19:00 | `drag-t3-p9-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_91133b | 2026-09-05 19:04 | `drag-t3-p10-window` | 0 | 0 | 0 | rendered | candidate |
| exp_e00eb8 | 2026-09-05 19:07 | `drag-t3-p10-screen` | 0 | 0 | 0 | rendered | candidate |
| exp_dbf64a | 2026-09-05 19:54 | `drag-s1-20260905-195227-r1-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_3e260d | 2026-09-05 19:57 | `drag-s1-20260905-195227-r1-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_ee2bb4 | 2026-09-05 20:01 | `drag-t3-20260905-195227-r1-s2b` | 0 | 0 | 0 | rendered | candidate |
| exp_56e3f3 | 2026-09-05 20:04 | `drag-s1-20260905-195227-r2-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_fce642 | 2026-09-05 20:07 | `drag-s1-20260905-195227-r2-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_68c20e | 2026-09-05 20:11 | `drag-t3-20260905-195227-r2-s2b` | 0 | 0 | 0 | rendered | candidate |
| exp_84a0cf | 2026-09-05 20:14 | `drag-s1-20260905-195227-r3-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_28df7d | 2026-09-05 20:17 | `drag-s1-20260905-195227-r3-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_b4da9d | 2026-09-05 20:21 | `drag-t3-20260905-195227-r3-s2b` | 0 | 0 | 0 | rendered | candidate |
| exp_d70578 | 2026-09-05 20:24 | `drag-s1-20260905-195227-r4-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_905505 | 2026-09-05 20:27 | `drag-s1-20260905-195227-r4-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_8d005e | 2026-09-05 20:31 | `drag-t3-20260905-195227-r4-s2b` | 0 | 0 | 0 | rendered | candidate |
| exp_3bf91a | 2026-09-05 20:34 | `drag-s1-20260905-195227-r5-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_f7482a | 2026-09-05 20:38 | `drag-s1-20260905-195227-r5-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_3dd6ca | 2026-09-05 20:41 | `drag-t3-20260905-195227-r5-s2b` | 0 | 0 | 0 | rendered | candidate |
| exp_93954c | 2026-09-05 23:54 | `drag-s1-20260905-235305-r1-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_a60606 | 2026-09-06 00:01 | `drag-t0-20260905-235305-r1-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_9174b4 | 2026-09-06 00:54 | `drag-s1-20260906-005246-r1-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_bf9252 | 2026-09-06 00:57 | `drag-t0-20260906-005246-r1-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_ee00ed | 2026-09-06 01:01 | `drag-s1-20260906-005246-r2-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_bb718c | 2026-09-06 01:04 | `drag-t0-20260906-005246-r2-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_5d8332 | 2026-09-06 01:06 | `drag-s1-20260906-005246-r3-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_4dd666 | 2026-09-06 01:06 | `drag-t0-20260906-005246-r3-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_284c55 | 2026-09-06 01:08 | `drag-s1-20260906-005246-r4-baseline` | 0 | 0 | 0 | black | candidate |
| exp_53958f | 2026-09-06 01:10 | `drag-s1-20260906-005246-r5-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_3043f8 | 2026-09-06 01:10 | `drag-s1-20260906-005246-r6-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_9e5d5c | 2026-09-06 01:10 | `drag-s1-20260906-005246-r7-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_45a15f | 2026-09-06 01:10 | `drag-s1-20260906-005246-r8-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_0af6ad | 2026-09-06 01:10 | `drag-s1-20260906-005246-r9-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_782e54 | 2026-09-06 01:10 | `drag-t0-20260906-005246-r4-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_591285 | 2026-09-06 01:10 | `drag-t0-20260906-005246-r5-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_db7092 | 2026-09-06 01:10 | `drag-t0-20260906-005246-r6-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_ba36ca | 2026-09-06 01:10 | `drag-t0-20260906-005246-r7-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_a581dd | 2026-09-06 01:10 | `drag-t0-20260906-005246-r8-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_daa151 | 2026-09-06 01:11 | `drag-s1-20260906-005246-r10-baseline` | 0 | 0 | 0 | — | no-capture |
| exp_c3b0c3 | 2026-09-06 01:11 | `drag-t0-20260906-005246-r10-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_8a9232 | 2026-09-06 01:11 | `drag-t0-20260906-005246-r9-s1diag` | 0 | 0 | 0 | — | no-capture |
| exp_e54805 | 2026-09-06 09:15 | `drag-s1-20260906-091345-r1-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_f17958 | 2026-09-06 09:18 | `drag-t0-20260906-091345-r1-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_9268b4 | 2026-09-06 09:22 | `drag-s1-20260906-091345-r2-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_c638aa | 2026-09-06 09:25 | `drag-t0-20260906-091345-r2-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_961792 | 2026-09-06 09:29 | `drag-s1-20260906-091345-r3-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_77fc9a | 2026-09-06 09:32 | `drag-t0-20260906-091345-r3-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_51066c | 2026-09-06 09:36 | `drag-s1-20260906-091345-r4-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_15336e | 2026-09-06 09:39 | `drag-t0-20260906-091345-r4-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_420605 | 2026-09-06 09:43 | `drag-s1-20260906-091345-r5-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_3169bf | 2026-09-06 09:46 | `drag-t0-20260906-091345-r5-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_68cecc | 2026-09-06 09:50 | `drag-s1-20260906-091345-r6-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_f1edcf | 2026-09-06 09:53 | `drag-t0-20260906-091345-r6-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_cd3590 | 2026-09-06 09:57 | `drag-s1-20260906-091345-r7-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_64c61c | 2026-09-06 10:00 | `drag-t0-20260906-091345-r7-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_4814e4 | 2026-09-06 10:04 | `drag-s1-20260906-091345-r8-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_780511 | 2026-09-06 10:07 | `drag-t0-20260906-091345-r8-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_5e9252 | 2026-09-06 10:11 | `drag-s1-20260906-091345-r9-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_856dd5 | 2026-09-06 10:14 | `drag-t0-20260906-091345-r9-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_f5a7dd | 2026-09-06 10:18 | `drag-s1-20260906-091345-r10-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_c2d5a1 | 2026-09-06 10:21 | `drag-t0-20260906-091345-r10-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_758ff6 | 2026-09-06 13:53 | `drag-t0-validate-20260906-135138-r1-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_e2dad8 | 2026-09-06 14:09 | `drag-s1-20260906-140732-r1-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_d4ebe1 | 2026-09-06 14:12 | `drag-s1-20260906-140732-r1-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_db79d7 | 2026-09-06 14:15 | `drag-s1-20260906-140732-r2-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_51a0e7 | 2026-09-06 14:19 | `drag-s1-20260906-140732-r2-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_95d66b | 2026-09-06 14:22 | `drag-s1-20260906-140732-r3-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_5e38db | 2026-09-06 14:26 | `drag-s1-20260906-140732-r3-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_ebb4ae | 2026-09-06 14:29 | `drag-s1-20260906-140732-r4-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_96097b | 2026-09-06 14:33 | `drag-s1-20260906-140732-r4-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_b1da4c | 2026-09-06 14:36 | `drag-s1-20260906-140732-r5-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_740386 | 2026-09-06 14:40 | `drag-s1-20260906-140732-r5-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_bf78f0 | 2026-09-06 14:43 | `drag-s1-20260906-140732-r6-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_dda082 | 2026-09-06 14:47 | `drag-s1-20260906-140732-r6-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_466147 | 2026-09-06 14:50 | `drag-s1-20260906-140732-r7-baseline` | 0 | 0 | 0 | rendered | candidate |
| exp_e38dd6 | 2026-09-06 14:53 | `drag-s1-20260906-140732-r7-stage1` | 0 | 0 | 0 | rendered | candidate |
| exp_375b91 | 2026-09-06 16:30 | `drag-t0-20260906-162828-r1-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_ac2da6 | 2026-09-06 16:33 | `drag-t0-20260906-162828-r2-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_f9299f | 2026-09-06 16:36 | `drag-t0-20260906-162828-r3-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_8596e7 | 2026-09-06 16:39 | `drag-t0-20260906-162828-r4-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_068fd0 | 2026-09-06 16:41 | `drag-t0-20260906-162828-r5-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_b475bd | 2026-09-06 16:44 | `drag-t0-20260906-162828-r6-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_12b061 | 2026-09-06 16:47 | `drag-t0-20260906-162828-r7-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_b4fb6f | 2026-09-06 16:50 | `drag-t0-20260906-162828-r8-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_09db5d | 2026-09-06 16:53 | `drag-t0-20260906-162828-r9-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_90d6ea | 2026-09-06 16:56 | `drag-t0-20260906-162828-r10-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_1d4687 | 2026-09-06 16:59 | `drag-t0-20260906-162828-r11-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_49a27a | 2026-09-06 17:02 | `drag-t0-20260906-162828-r12-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_3864a3 | 2026-09-06 23:20 | `drag-t0-20260906-231838-r1-s1diag` | 0 | 0 | 0 | rendered | candidate |
| exp_c72a8f | 2026-09-06 23:23 | `drag-s1-20260906-232215-r1-t0b` | 0 | 0 | 0 | rendered | candidate |
| exp_c8ed8e | 2026-09-06 23:26 | `drag-s1-20260906-232215-r2-t0b` | 0 | 0 | 0 | rendered | candidate |
| exp_a8017e | 2026-09-06 23:29 | `drag-s1-20260906-232215-r3-t0b` | 0 | 0 | 0 | rendered | candidate |
| exp_124286 | 2026-09-06 23:32 | `drag-s1-20260906-232215-r4-t0b` | 0 | 0 | 0 | rendered | candidate |
| exp_cb0d6d | 2026-09-06 23:35 | `drag-s1-20260906-232215-r5-t0b` | 0 | 0 | 0 | rendered | candidate |
| exp_e0b58f | 2026-09-06 23:38 | `drag-s1-20260906-232215-r6-t0b` | 0 | 0 | 0 | rendered | candidate |
| exp_25fa82 | 2026-09-06 23:41 | `drag-s1-20260906-232215-r7-t0b` | 0 | 0 | 0 | rendered | candidate |
...
| exp_66e91e | 2026-09-07 19:40 | `drag-s1-20260907-193827-r1-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_9ecac7 | 2026-09-07 19:42 | `drag-s1-20260907-193827-r2-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_7e3ba8 | 2026-09-07 19:46 | `drag-s1-20260907-193827-r3-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_5886dd | 2026-09-07 19:49 | `drag-s1-20260907-193827-r4-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_92f19f | 2026-09-07 19:52 | `drag-s1-20260907-193827-r5-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_aa547d | 2026-09-07 19:55 | `drag-s1-20260907-193827-r6-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_995321 | 2026-09-07 19:58 | `drag-s1-20260907-193827-r7-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_958338 | 2026-09-07 20:01 | `drag-s1-20260907-193827-r8-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_6a0660 | 2026-09-07 20:04 | `drag-s1-20260907-193827-r9-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_2eecf0 | 2026-09-07 20:07 | `drag-s1-20260907-193827-r10-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_cc0946 | 2026-09-07 20:10 | `drag-s1-20260907-193827-r11-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_a86345 | 2026-09-07 20:13 | `drag-s1-20260907-193827-r12-s0-adrop-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_f7a9b6 | 2026-09-07 21:24 | `drag-s1-20260907-212248-r1-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_b8f4a2 | 2026-09-07 21:29 | `drag-s1-20260907-212248-r2-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_69f99c | 2026-09-07 21:32 | `drag-s1-20260907-212248-r3-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_e96951 | 2026-09-07 21:35 | `drag-s1-20260907-212248-r4-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_f179db | 2026-09-07 21:38 | `drag-s1-20260907-212248-r5-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_1f7380 | 2026-09-07 21:41 | `drag-s1-20260907-212248-r6-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_02d65c | 2026-09-07 21:44 | `drag-s1-20260907-212248-r7-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_ec136f | 2026-09-07 21:47 | `drag-s1-20260907-212248-r8-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_145364 | 2026-09-07 21:50 | `drag-s1-20260907-212248-r9-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_024f02 | 2026-09-07 21:53 | `drag-s1-20260907-212248-r10-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_2bcda2 | 2026-09-07 21:56 | `drag-s1-20260907-212248-r11-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |
| exp_a85142 | 2026-09-07 21:59 | `drag-s1-20260907-212248-r12-s0-mutant-cyan` | 0 | 0 | 0 | rendered | candidate |

301 cells · 45 VOID-LIBS · 241 candidate
---
