...
| exp_374ae7 | 2026-09-03 17:05 | `classbg` | 0 | 0 | 0 | rendered | candidate |
...
| exp_5c1a70 | 2026-09-03 18:34 | `t1-before` | 0 | 0 | 0 | rendered | candidate |
| exp_e42b19 | 2026-09-03 18:37 | `t1-after` | 0 | 0 | 0 | rendered | candidate |
| exp_7d0f4c | 2026-09-03 19:33 | `stage1` | 0 | 0 | 0 | rendered | candidate |

> ⚠ The interleaved A/B (C38) ran **four** Steam sessions under the one cell label
> `stage1`, so the evidence store holds only the last one's fingerprint. The per-session frames,
> band scores and traces are in `~/cs2-patch/stage1-ab2/<module>-r<round>/`, which is what C38
> cites; the module hash is printed in each session's header line. Both modules were built from the
> same tree state, so the one fingerprint is representative of the run's preconditions but NOT of
> which module produced which frames — read that from the run directories.

112 cells · 45 VOID-LIBS · 67 candidate
---
