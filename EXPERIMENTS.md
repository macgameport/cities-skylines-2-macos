...
| C57 | **The host layer normally trails a growing window by 50-100 px, and in 1 run in 20 it does not track a whole grow segment at all — sitting at the pre-drag width for 7.2 s while the window grows 650 px** | `SUPPORTED` | Re-analysis of traces already on disk; **no new runs**. `scripts/host-frame-stalls.py` over all **42 completed runs** in `~/cs2-patch/strip-ab` (19 baseline, 13 stage-1 diag, 5 stage 1, 5 stage 2 `s2b`), every one a `FRAMES=300` whole-drag row at [[C50]]'s cadence (25 px / 120 ms, `PRESIZE=1000x650`, right +650 x3, top -350) with `TRACE=+err,+macdrv,+cursor,+timestamp`. Metric: the widest single install-to-install GROW of one child's hosted frame, in time order, excluding a host's first `0 -> full size` install (the create path) and jumps completing under 1 s. The two runs that show the stall are cells **exp_56e3f3** (`drag-s1-20260905-195227-r2-baseline`, baseline `cd79fc463795939f`) and **exp_bb718c** (`drag-t0-20260906-005246-r2-s1diag`, diag `50fdfe79898dac36`) | **Measured — the distribution is bimodal with nothing in between.** 40 runs track to within **50 px (17), 75 px (20) or 100 px (3)**; 2 runs jump **650 px in a single install after 7.2 s**. No run lies between 100 and 650 px, so this is a distinct failure mode and not the tail of the ordinary lag. **Rate 2 of 42 (5%).** **Corroborated independently from the pixels, which were scored without reference to the trace:** on both runs the captured frames give a CONSTANT solved content width across the episode — dark% = (W - x)/W yields x = **1006, 1006, 1007 px** over 5, 10 and 17 consecutive frames while the window grows to 1660 — matching the trace's stalled install width to within a pixel. The exposed region carries **0.0 % of every diagnostic colour** on the diag arm, i.e. it is [[C39]]'s S4, at up to 40 % of the window for ~1.9 s. Also measured, on the same traces: after retiring a superseded layer the host takes a median **100 ms** to install the replacement's frame (`scripts/layer-gap.py`, n=260 on one diag run). **This names [[C54]]'s and [[C55]]'s unexplained outliers.** C54 recorded baseline growing-frame means of 6.3 / **43.3** / 6.8 / 6.3 / 4.3 % and C55 a diag arm of 5.0 / **39.3** %, both treated as run-to-run spread; the two high runs are exactly the two stall runs, and the spread was a mechanism. **Inferred, not measured:** what causes a segment to be skipped. The trace shows the host was never asked — `update_remote_layer_frame_for` does not run — but nothing here separates a child that stopped posting from a host that dropped the messages. Also unclaimed: whether the rate differs by module (1 of 19 baseline, 1 of 13 diag, 0 of 10 across both stage builds is nowhere near separable), though [[C55]]'s architectural reading predicts it does not. ⚠ **The detector is easy to get wrong in three ways and each was made here first:** grouping installs by child+context scores 0 of 42 (every resize step mints a new context, so the width progression lives across them), pooling all children scores 86 % (one window's children legitimately differ in size), and counting the create path as a stall inflates every run. **What would overturn it:** ~20 further whole-drag runs with no 650 px install jump, which would put the two occurrences down to something outside the drag. Tracked in issue #7. |
...
227 cells · 45 VOID-LIBS · 167 candidate
---
