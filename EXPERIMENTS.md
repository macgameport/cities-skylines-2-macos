...
| C28 | Live-drag flicker is **minimal** after the shimmer fix | `UNREVIEWED` | James's assessment by eye, 2026-08-31 | The only assessment available: `shimmer-probe.sh` drives `SetWindowPos`, while a human dragging a window edge goes through macOS live-resize, a path the harness cannot reach. **Deliberately filed `UNREVIEWED`, not `SUPPORTED`** — one person's eyes on one drag is not a measurement, and promoting it would be the same move this project has had to retract before. Measuring it is feasible (pointer position is readable without permissions, so captures could be sampled during a real drag) and has not been done. Overturned by: anyone finding it obvious in normal use. |
